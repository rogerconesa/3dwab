...
function(declare, lang, array, html, topic, domConstruct, domGeometry,
  ...
  clazz = declare([BaseLayoutManager], {
    ...
    setSceneView: function(sceneView){
      this.inherited(arguments);
      this.panelManager.setSceneView(sceneView);
    },
    ...
    onLayoutChange: function(appConfig){
      ...
      array.forEach(this.onScreenGroupPanels, function(panel){
        var configElement = appConfig.getConfigElementById(panel.config.id);
        if(configElement && configElement.panel) {
          var position = configElement.panel.position;
