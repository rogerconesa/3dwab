...
define(['dojo/_base/declare',
  ...
  function(declare, lang, html, on, query, cookie, _WidgetsInTemplateMixin, BaseWidget,
           CheckBox, utils, LoadingShelter, Deferred, EditorXssFilter, EditorPreviewLinkMessager) {
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      onOpen: function() {
        if (!utils.isInConfigOrPreviewWindow()) {
          ...
          if (utils.isDefined(isfirst) && isfirst.toString() === 'false') {
            this.close();
          }
        }
          ...
          if (this.customContentNode.nodeType && this.customContentNode.nodeType === 1) {
            ...
            if (contentImgs && contentImgs.length) {
              for (var i = 0, len = contentImgs.length; i < len; i++) {
                ...
                html.setStyle(img, 'maxWidth', (customBox.w - 40 - 20) + 'px');// prevent x scroll
                html.setStyle(img, 'maxHeight', (customBox.h - 40) + 'px');
              }
            }
          }
        }
